...
?>
	...
	<?php endif; ?>
		...
		<div id="wp-subscribe"  class="newsletter wp-subscribe-wrap wp-subscribe wp-subscribe-<?php echo $wp_subscribe_forms ?>" data-thanks_page="<?php echo absint( $options['thanks_page'] ) ?>" data-thanks_page_url="<?php echo esc_url( $options['thanks_page_url'] ) ?>" data-thanks_page_new_window="0">

<!--			<h4 class="title">--><?php //echo wp_kses_post( $options['title'] )?><!--</h4>-->

<!--			<p class="text">--><?php //echo wp_kses_post( $options['text'] ) ?><!--</p>-->

			<?php if( method_exists( $service, 'the_form' ) && false ) :
				$service->the_form( $wp_subscribe_forms, $options );
			else: ?>

			<form action="<?php echo $current_url ?>" method="post" class="newsletter__form wp-subscribe-form wp-subscribe-<?php echo $options['service'] ?>" id="wp-subscribe-form-<?php echo $wp_subscribe_forms ?>">
...
<!--				<input class="submit" type="submit" name="submit" value="--><?php //echo esc_attr( $options['button_text'] ) ?><!--">-->
                ...
                <span class="input newsletter__input i-bem" data-bem='{"input":{}}'>
                    <span class="input__box"><input class="input__control" name="email" placeholder="<?php echo esc_attr( $options['email_placeholder'] ) ?>"></span>
                </span>
                <button class="button button__control newsletter__button fa fa_icon_paper-plane i-bem" data-bem='{"button":{}}' role="button" type="submit"><?php echo esc_attr( $options['button_text'] ) ?></button>
